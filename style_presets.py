...
2. 绘制动漫角色或已有二次元 IP 人物时，默认采用干净的日系二次元插画语言：准确利落的线稿或边缘、稳定的赛璐璐/平涂关系、通透肤色、统一配色。只有用户原话明确要求写实摄影、油画或厚涂时才更换媒介。
...
5. 日系动漫、漫画或游戏角色使用官方日文作品名和角色名；画内需要标题或装饰文字时只使用准确的官方日文原文，或用户明确指定的文字，不使用中文译名或中文装饰字。不要擅自添加无意义文字。
6. 若题材不是二次元人物（例如商品、美食、建筑或用户明确要求摄影），选择适合题材的专业视觉语言，但仍保持干净、克制和清晰层级。
7. “masterpiece、best quality、highly detailed、anime style、cinematic、8k、插画、高清”等只是泛化质量词，不属于用户指定的完整画风，不能用它们作为跳过内置风格的理由。
""".strip()
...
def build_style_guidance(
    ...
    *,
    ...
    lines = [
        ...
        "2. 用户原话明确给出具体且有辨识度的其他画风时，忠实保留并停止自动叠加内置风格。",
        "3. 只有当用户原话明确把选择权交给模型（例如“换种风格”“再换一个”）时，才可采纳工具草稿中具体、有辨识度且符合全局审美要求的风格，并优先映射到相符的内置风格；其他情况下工具草稿擅自推断的画风不算用户指定。anime style、highly detailed、masterpiece、cinematic、插画、高清等泛化词始终不算具体风格。",
        "4. 用户未指定具体画风、也未委托工具草稿选择时，必须主动选择一个相容的内置风格；优先完整采用它的核心视觉语言，仅在完整风格确实与主体或硬性要求冲突时才借用最匹配的构图、色彩、线条或材质特征。",
        "5. 多个风格都适合时，优先选择标记为“自动偏好：高”的风格；只有用户原话明确要求纯写实、忠实复刻原画风、不要风格化，或全部候选都明显冲突时才选择零个。",
        "6. 最多选择一个风格，不混合多个内置风格。用户的主体身份、品牌、商品、数量、动作、场景、构图和配色等明确要求永远优先；用户要求画进图片的文字必须逐字保留。",
        "7. 只吸收所选风格中适用的视觉属性，不复制示例主体，不添加用户未要求的角色、品牌、文字或物件。",
    ...
    if has_image:
        lines.append("12. 当前是改图：除非用户明确点名风格或明确要求整体重绘/风格化，否则不要改变原图画风。")
    if explicit:
