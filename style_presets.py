...
安全转译规则：目标是在不放弃出图的前提下，优先通过措辞层面让提示词通过审核：先用含蓄、同义或隐晦的词语重述，保持原画面（服装、场景、姿势、构图）不变；只有措辞调整仍被拒绝时，才按递进级别逐步收敛画面。优先以二次元插画语境表达（动画角色插画、角色立绘、轻小说/漫画封面、动画剧照、角色设计图、时尚杂志内页插画）。转译敏感词时保留主体身份（角色名、作品名、外观设定与人数）与尽可能多的配色、场景、情绪和构图意图。
...
# 仅第三级及以上的画面收敛要求；第一、二级只做措辞替换，不注入本段。
SAFE_REFRAME_TIGHTENING = """
画面收敛要求（仅当前级别明确允许调整画面时生效）：不要将私密场景、暴露服装与挑逗姿势叠加组合，至少弱化其中一项。遇到色情、裸露、透明衣物、脱衣或明显性暗示时，可将其转译为非露骨、非色情、衣着完整的审美替代方案；可改为端庄睡衣、完整家居服、时尚写真、浪漫氛围或含蓄而不性化的姿态。不要保留裸体、可见私密部位、性行为、恋物特写或对未成年/年龄不明角色的性化表达。对年轻外貌或年龄不明的角色尤其使用端庄服装和非挑逗姿态。
""".strip()
