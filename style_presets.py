...
def build_style_guidance(
    ...
    *,
    ...
    lines = [
        ...
        "2. 用户已给出具体而完整的其他风格时，忠实保留，不再自动叠加内置风格。",
        "3. 用户未指定风格时，应主动选择一个相容的内置风格；若完整风格不完全适合，可以只借用它最匹配的构图、色彩、线条或材质特征。",
        "4. 多个风格都适合时，优先选择标记为“自动偏好：高”的风格；只有用户明确要求无风格、纯写实，或全部候选都明显冲突时才选择零个。",
        ...
        "7. 把风格自然融入最终提示词，不输出风格 id、选择过程、解释或元信息。",
