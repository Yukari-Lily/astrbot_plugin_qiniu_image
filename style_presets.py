...
def build_style_guidance(
    ...
    *,
    ...
    lines = [
        ...
        "5. 自动选择时先从标记为‘自动偏好：高’的相容风格中选择一个；仅当高偏好项全部不相容才考虑普通项。只有用户明确要求其他具体画风、纯写实、忠实复刻原画风、不要风格化，或保留原图/风格参考、全部候选都明显冲突时才选择零个。",
    ...
    if drawing_task is not None and mode == "auto":
        lines.append(
            ...
            "user_requirements、changes 中用户明确指定的画风最高，随后是目标作品保留项和 style 图片参考。"
