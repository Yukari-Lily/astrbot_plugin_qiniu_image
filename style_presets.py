...
def build_style_guidance(
    ...
    *,
    ...
    lines = [
        ...
        "3. 工具草稿中的画风、媒介、构图和视觉效果不代表用户意图，不得据此选用风格。只有用户原话明确把选择权交给模型（例如“换种风格”“再换一个”）时，才由当前改写模型依据本风格库独立选择相容的内置风格。anime style、highly detailed、masterpiece、cinematic、插画、高清等泛化词始终不算具体风格。",
        "4. 用户未指定具体画风时，必须主动选择一个相容的内置风格；优先完整采用它的核心视觉语言，仅在完整风格确实与主体或硬性要求冲突时才借用最匹配的构图、色彩、线条或材质特征。",
